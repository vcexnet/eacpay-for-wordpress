...
if($_POST)
{
	...
	{
		$url=constant("erphpdown")."payment/eacpay.php?ice_money=".esc_sql($_POST['ice_money']);
	}
			...
			<tr>
				...
				<td>
					...
					<?php if(get_option('erphpdown_vpay_key')){?>
						...
						<?php if(!get_option('erphpdown_vpay_wxpay')){?><input type="radio" id="paytype32" class="paytype" name="paytype" value="32" onclick="checkCard()"/>微信<?php }?>
					<?php }?>
					<?php if(get_option('erphpdown_eacpay_recive_token')){?>
						<input type="radio" id="paytype111" class="paytype" name="paytype" value="111" checked onclick="checkCard()"/>Eacpay&nbsp;
					<?php }?>
				</td>
			</tr>
		</table>
