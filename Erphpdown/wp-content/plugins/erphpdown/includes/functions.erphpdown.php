...
function mobantu_erphp_menu() {
	...
	if($token){
		...
		if (function_exists('add_submenu_page')) {
			...
			add_submenu_page('erphpdown/admin/erphp-settings.php', '购买统计(登录)', '购买统计(登录)', 'activate_plugins', 'erphpdown/admin/erphp-orders-list.php');
			//add_submenu_page('erphpdown/admin/erphp-settings.php', '附加购买统计', '附加购买统计', 'activate_plugins', 'erphpdown/admin/erphp-indexs-list.php');
			add_submenu_page('erphpdown/admin/erphp-settings.php', '购买统计(免登录)', '购买统计(免登录)', 'activate_plugins', 'erphpdown/admin/erphp-wppays-list.php');
	        ...
	        if(plugin_check_card()){
				add_submenu_page('erphpdown/admin/erphp-settings.php', '所有充值卡','所有充值卡', 'activate_plugins', 'erphpdown/addon/card/list.php');
			...
			if(plugin_check_activation()){
				add_submenu_page('erphpdown/admin/erphp-settings.php', '所有激活码','所有激活码', 'activate_plugins', 'erphpdown/addon/activation/list.php');
			...
			if(plugin_check_cred()){
				add_submenu_page('erphpdown/admin/erphp-my-money.php', '积分兑换','积分兑换', 'read', 'erphpdown/addon/mycred/erphp-to-mycred.php');
			}
			...
			if($okMoney >= $price*get_option('ice_proportion_alipay')){
				if($wppay->addWppay($out_trade_no, $price)){
					...
					if($result){
						...
						$EPD = new EPD();
						$EPD->doAff($price*get_option('ice_proportion_alipay'), $user_id);

						if(get_option('erphp_remind')){
							$subject   = get_post($post_id)->post_title;
							wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$subject, '用户'.$current_user->user_login.'消费'.$price.'元购买了'.$subject.get_permalink($post_id));
						}
					}
	...
	if($post_id && $order_num){
		...
		if($wppay->checkWppayPaid($order_num)){
		    ...
		    if(isset($_COOKIE["erphprefid"]) && is_numeric($_COOKIE["erphprefid"])){
		        $post_price = $wpdb->get_var("select post_price from $wppay_table_name where order_num='".$order_num."'");

		        $EPD = new EPD();
				$EPD->doAff2($post_price*get_option('ice_proportion_alipay'), $_COOKIE["erphprefid"]);

				if(get_option('erphp_remind')){
					$subject   = get_post($post_id)->post_title;
					wp_mail(get_option('admin_email'), '【'.get_bloginfo('name').'】订单提醒 - '.$subject, '免登录用户消费'.$post_price.'元购买了'.$subject.get_permalink($post_id));
				}
...
function erphpdown_install() {
	...
	$create_ice_money_sql="CREATE TABLE $wpdb->icemoney (".
			...
			"eac float(10, 4) DEFAULT '0',".
			"real_eac float(10, 4) DEFAULT '0',".
			"block_height varchar(32) DEFAULT NULL,".
			"`last_time` int(11) NULL DEFAULT 0,".
			"`status` enum('cancel','reject','wait','complete','payed') DEFAULT 'wait',".
			"`type` enum('recharge','cash') DEFAULT NULL,".
			"PRIMARY KEY (ice_id)) $charset_collate;";
	...
	$create_get_money_sql="CREATE TABLE $wpdb->iceget (".
			...
			"ice_note varchar(50) NOT NULL,".
			"ice_success_time datetime NOT NULL,".
			"ice_payment varchar(50) default NULL,".
			"eac float(10,4) default 0,".
			"PRIMARY KEY (ice_id)) $charset_collate;";
	...
	dbDelta($create_ice_card_sql);

	$create_iceactivation_sql = "CREATE TABLE $wpdb->erphpact (
	...
	dbDelta($create_iceactivation_sql);
