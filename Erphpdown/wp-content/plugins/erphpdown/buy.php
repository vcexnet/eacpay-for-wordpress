			...
			if($downInfo && !$down_repeat){
				...
				if($index){
					...
					if($urls){
            			...
            			if($cnt){
							...
							if($price){
								...
								if( $indexMemberDown==4 || $indexMemberDown==8 || $indexMemberDown==9 || (($indexMemberDown == 10 || $indexMemberDown == 11 || $indexMemberDown == 12) && !$userType))
								{
									echo "您无权购买此资源！";exit;
								}
								...
								<table class="erphpdown-table" width="100%" align="center">
									<tr>
										<td style="border-bottom: 1px solid #eee;"><div style="display:inline-block;max-height:48px;overflow:hidden;"><span>名称：</span><?php echo get_post($postid)->post_title;?> - <?php echo $index_name;?></div></td>
									</tr>
									<tr>
										<td style="border-bottom: 1px solid #eee;"><span>价格：</span><?php echo sprintf("%.2f",$price);?><?php echo  $vip==TRUE?' <del>(原价:'.sprintf("%.2f",$price_old).')</del>' :'';?> <?php echo get_option('ice_name_alipay');?></td>
									</tr>
									...
									<tr>
										...
										<?php if(sprintf("%.2f",$okMoney) >= sprintf("%.2f",$price) && !$down_only_pay) {?>
											...
											<button class="erphpdown-btn do-erphpdown-pay" data-href="<?php echo constant("erphpdown").'checkout.php?ajax=1&postid='.$postid;?>&index=<?php echo $index;?>" style="border:none;cursor: pointer;">使用余额支付</button>
											<?php }else{?>
										...
										?>
											...
											<?php if(get_option('erphpdown_payjs_appid')){?> 
												<a href="<?php echo constant("erphpdown")."payment/payjs.php?ice_post=".$postid."&index=".$index."&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-wx erphpdown-jump" target="_blank"><i class="erphp-iconfont erphp-icon-wxpay"></i> 微信支付</a>
												<a href="<?php echo constant("erphpdown")."payment/payjs.php?ice_post=".$postid."&index=".$index."&type=alipay&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-ali erphpdown-jump" target="_blank"><i class="erphp-iconfont erphp-icon-alipay"></i> 支付宝</a>   
											<?php }?>
											...
											<?php if(get_option('erphpdown_eacpay_recive_token')){?> 
												<a href="<?php echo constant("erphpdown")."payment/eacpay.php?ice_post=".$postid."&index=".$index."&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-eacpay erphpdown-jump" target="_blank" style="background: #2c8ef1;"><i class="erphp-iconfont erphp-icon-eacpay" style="background: url(static/images/payment-eacpay.png);width: 16px;height: 16px;display: inline-block;background-size: cover;vertical-align: bottom;"></i> Eacpay支付</a>
											<?php }?>
					...
					if($price){
						...
						<table class="erphpdown-table" width="100%" align="center">
							<tr>
								<td style="border-bottom: 1px solid #eee;"><div style="display:inline-block;max-height:48px;overflow:hidden;"><span>名称：</span><?php echo get_post($postid)->post_title;?></div></td>
							</tr>
							<tr>
								<td style="border-bottom: 1px solid #eee;"><span>价格：</span><?php echo sprintf("%.2f",$price);?><?php echo  $vip==TRUE?' <del>(原价:'.sprintf("%.2f",$price_old).')</del>' :'';?> <?php echo get_option('ice_name_alipay');?></td>
							</tr>
							...
							<tr>
								...
								<?php if(sprintf("%.2f",$okMoney) >= sprintf("%.2f",$price) && !$down_only_pay) {?>
									<div style="margin-top: 50px;">
									<?php if($erphp_ajaxbuy){?>
									<button class="erphpdown-btn do-erphpdown-pay" data-href="<?php echo constant("erphpdown").'checkout.php?ajax=1&postid='.$postid;?>" style="border:none;cursor: pointer;">使用余额支付</button>
									<?php }else{?>
								...
								?>
									...
									<?php }?>
									<?php if(get_option('erphpdown_payjs_appid')){?> 
										<a href="<?php echo constant("erphpdown")."payment/payjs.php?ice_post=".$postid."&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-wx erphpdown-jump" target="_blank"><i class="erphp-iconfont erphp-icon-wxpay"></i> 微信支付</a>
										<a href="<?php echo constant("erphpdown")."payment/payjs.php?ice_post=".$postid."&type=alipay&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-ali erphpdown-jump" target="_blank"><i class="erphp-iconfont erphp-icon-alipay"></i> 支付宝</a>   
									<?php }?>
									<?php if(get_option('erphpdown_xhpay_appid31')){?> 
									...
									<?php }?>
									<?php if(get_option('erphpdown_eacpay_recive_token')){?> 
										<a href="<?php echo constant("erphpdown")."payment/eacpay.php?ice_post=".$postid."&redirect_url=".urlencode(get_permalink($postid));?>" class="pmt-eacpay erphpdown-jump" target="_blank" style="background: #2c8ef1;"><i class="erphp-iconfont erphp-icon-eacpay" style="background: url(static/images/payment-eacpay.png);width: 16px;height: 16px;display: inline-block;background-size: cover;vertical-align: bottom;"></i> Eacpay支付</a>
									<?php }?>
									<?php if(get_option('erphpdown_stripe_pk') && plugin_check_stripe()){?> 
